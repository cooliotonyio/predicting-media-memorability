...
def average_predictions(predictions, actuals, video_ids):
    ...
    actuals_per_video = {vid: [] for vid in np.unique(video_ids)} 
...
def build_matrixes(data, target_name, feature_name, dtype=np.float32):
    ...
    for video_id, row in data.iterrows():
        ...
        if len(feature) == 0:
            print(f"WARNING: Video '{video_id}' has no '{feature_name}' features")
            continue
...
def get_predictions(model_type, model, features, targets, video_ids):
    ...
    return average_predictions(predictions, actuals, video_ids)
...
def train_two_layer_nn(features_train, 
    ...
    for epoch in range(num_epochs):
        print(f"--------------- Epoch {epoch} ----------------") if verbose else 0
...
def get_nn_predictions(model, features_valid, targets_valid, cuda = True):
    ...
    device = torch.device("cuda") if cuda else torch.device("cpu")
    dataloader = torch.utils.data.DataLoader(FlatDataset(features_valid, targets_valid), batch_size=1)
