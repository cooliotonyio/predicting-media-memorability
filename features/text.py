    ...
    for video_id in video_ids:
        ...
        if os.path.exists(video_dir) and len(os.listdir(video_dir)) > 0:
            for caption_feature_file in np.sort(os.listdir(video_dir)):
                caption_feature = np.loadtxt(f"{video_dir}/{caption_feature_file}", delimiter = ",")
...
def build_extractor(word_index, word_embeddings, embedding_dim, max_sequence_length):
    ...
    embedding_matrix = build_embedding_matrix(word_index, word_embeddings, embedding_dim)
...
def extract_text_features(video_ids, captions, tokenizer, word_embeddings, feature_dir, embedding_dim):

    if not os.path.exists(feature_dir):
        os.mkdir(feature_dir)
        
    sequence_lengths = [len(caption.split()) for caption in captions]
    max_sequence_length = np.max([len(caption.split()) for caption in captions])
    sequences = pad_sequences(tokenizer.texts_to_sequences(captions), maxlen = max_sequence_length)
    
    print("Max sequence length:", max_sequence_length)
    print("Min sequence length:", np.min(sequence_lengths))
    print("Avg sequence length:", np.mean(sequence_lengths))
    
    extractor = build_extractor(tokenizer.word_index, word_embeddings, embedding_dim, max_sequence_length)
    features = extractor.predict(sequences)
    
    for video_id, feature in zip(video_ids, features):
        video_feature_dir = f"{feature_dir}/{video_id}"
        
        if not os.path.exists(video_feature_dir):
            os.mkdir(video_feature_dir)
            
        i = len(os.listdir(video_feature_dir)) # i-th extracted caption of the video
        np.savetxt(f"{video_feature_dir}/{i}.csv", feature, delimiter = ",")
        
def extract_GLoVe_features(video_ids, captions, tokenizer, feature_dir = GLOVE_FEATURE_DIR, embedding_dim = GLOVE_WORD_EMBEDDING_DIM):
    word_embeddings = load_GLoVe_word_embeddings()
    extract_text_features(video_ids, captions, tokenizer, word_embeddings, feature_dir, embedding_dim)
